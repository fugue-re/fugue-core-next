...
DEFAULT_POSSIBLE_CONFIDENCE = 0.6
...
def format_pattern(bytes, mask):
    ...
    for i, (v, m) in enumerate(zip(bytes, mask)):
        ...
        if m == 0x00:
            ...
            # Low nibble is significant (mask 0x0f), high nibble is wildcard
            pat += "."
            pat += HTABLE[v & 0xf]
        elif m == 0xf0:
            # High nibble is significant (mask 0xf0), low nibble is wildcard
            pat += HTABLE[v >> 4]
            pat += "."
        elif m == 0xff:
            pat += HTABLE[v >> 4]
...
def _convert_ghidra_pattern_to_image_and_mask(ghidra_pattern):
    ...
    for i in range(len(ghidra_pattern)):
        ...
        if "0x" in pattern_item[0:2]:
            ...
            if len(pattern_item) % 2 == 0 and len(pattern_item) >= 2:
                cur_image, cur_mask = _parse_ghidra_pattern_item(
                    pattern_item, 4)

                # n byte format (any even number of hex digits)
...
def parse_pattern(elt: etree.Element):
    ...
    for elt in elt.iterchildren():
        ...
        elif elt.tag == "funcstart":
            confidence = DEFAULT_CONFIDENCE
        elif elt.tag == "possiblefuncstart":
            confidence = DEFAULT_POSSIBLE_CONFIDENCE
...
def parse(path, language, author, version):
...
@click.option("--language", "language", default="*:*:*:*:*", help="language constraint (format: processor:endian:bitness:variant:convention)")
@click.option("--author", "author", default="National Security Agency (NSA)", help="author of the patterns")
@click.option("--version", "version", default="0.1.0", help="version of the patterns")
def main(input, output, language, author, version):
    v = parse(input, language, author, version)
