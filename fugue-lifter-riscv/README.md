...
Fugue lifters for RISC-V.

## Build options

By default, both targets (RV32 and RV64, little endian) are built and
pre-transpiled lifters are used for faster compile times. To generate lifters
from source, use the following:

```sh
cargo build --features=compiled,riscv,riscv64 --no-default-features
```
...
cargo run --bin lifter-packager -- build-static \
    --language-db ./fugue-lifter-riscv/data/processors \
    --language RISCV:LE:32:default \
    --output ./fugue-lifter-riscv/data/generated/riscv.rs.gz

cargo run --bin lifter-packager -- build-static \
    --language-db ./fugue-lifter-riscv/data/processors \
    --language RISCV:LE:64:default \
    --output ./fugue-lifter-riscv/data/generated/riscv64.rs.gz
```

Both targets share the same `.sinc` sources, so regenerate **both** bundles
whenever the source tree (or `data/patches/`) changes — otherwise the outdated
bundle will collide with the fresh one at compile time.
...
- **Paths in the diff are relative to `data/processors/`**, not the repository
  ...
  ```sh
  cd fugue-lifter-riscv/data/processors
  git diff -- RISCV/riscv.instr.sinc > ../patches/0001-your-fix.patch
  ```

  A `git diff` taken from the repository root produces headers like
  `a/fugue-lifter-riscv/data/processors/RISCV/...`, which the patcher rejects
...
- After editing `data/patches/`, regenerate both bundles (see above) so the
