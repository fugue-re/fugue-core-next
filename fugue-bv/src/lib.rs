...
use malachite::base::num::arithmetic::traits::{
    ExtendedGcd, Gcd, Lcm, ModPowerOf2, ModPowerOf2Assign, PowerOf2,
};
...
fn expect_buffer_size(len: usize, bits: u32) {
    let size = bits.div_ceil(8) as usize;
    if len != size {
        panic!("invalid buffer size {len}; expected {size}");
    }
}
...
impl FromStr for BitVec {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let value = if let Some(cstv) = cst.strip_prefix("0x") {
        ...
        Ok(Self::from_bigint(value, bits))
    }
}
...
impl BitVec {
    ...
    pub fn from_str_radix(s: &str, radix: u32) -> Result<Self, ParseError> {
        let (cst, sz) = s.rsplit_once(':').ok_or(ParseError::InvalidFormat)?;
        let value = BigInt::from_string_base(radix as u8, cst).ok_or(ParseError::InvalidConst)?;
    ...
    pub fn from_be_bytes(buffer: &[u8]) -> Self {
        Self::from_be_bytes_with(buffer, buffer.len() as u32 * 8)
    }

    pub fn from_be_bytes_with(buffer: &[u8], bits: u32) -> Self {
        let meta = Self::pack_meta(false, bits);
        expect_buffer_size(buffer.len(), bits);
        if bits <= 64 {
            let mut raw = [0u8; 8];
            raw[8 - buffer.len()..].copy_from_slice(buffer);
            Self::from_small(u64::from_be_bytes(raw), meta)
        } else {
            let value = Natural::from_power_of_2_digits_desc(8, buffer.iter().copied())
                .expect("bytes are valid base-256 digits");
            Self::from_large(value, meta)
        }
    }

    pub fn from_le_bytes(buffer: &[u8]) -> Self {
        Self::from_le_bytes_with(buffer, buffer.len() as u32 * 8)
    }

    pub fn from_le_bytes_with(buffer: &[u8], bits: u32) -> Self {
        let meta = Self::pack_meta(false, bits);
        expect_buffer_size(buffer.len(), bits);
        if bits <= 64 {
            let mut raw = [0u8; 8];
            ...
            let value = Natural::from_power_of_2_digits_asc(8, buffer.iter().copied())
                .expect("bytes are valid base-256 digits");
            Self::from_large(value, meta)
        }
    }
    ...
    #[inline(always)]
    pub fn from_ne_bytes(buffer: &[u8]) -> Self {
    ...
    #[inline(always)]
    pub fn from_ne_bytes_with(buffer: &[u8], bits: u32) -> Self {
        if cfg!(target_endian = "big") {
            Self::from_be_bytes_with(buffer, bits)
        } else {
            Self::from_le_bytes_with(buffer, bits)
        }
    }
    ...
    fn expect_same_bits(&self, rhs: &Self, operation: &str) {
        if self.bits() != rhs.bits() {
            panic!(
                "cannot use `{operation}` with bit vector of size {} and bit vector of size {}",
                self.bits(),
                rhs.bits()
            );
        }
    ...
    pub fn to_be_bytes(&self, buffer: &mut [u8]) {
        expect_buffer_size(buffer.len(), self.bits());
        if self.is_small() {
            buffer.copy_from_slice(&self.small().to_be_bytes()[8 - buffer.len()..]);
        } else {
            let digits = self.large().to_power_of_2_digits_desc(8);
            let split = buffer.len() - digits.len();
            buffer[..split].fill(0);
            buffer[split..].copy_from_slice(&digits);
        }
    }

    pub fn to_le_bytes(&self, buffer: &mut [u8]) {
        expect_buffer_size(buffer.len(), self.bits());
        if self.is_small() {
            buffer.copy_from_slice(&self.small().to_le_bytes()[..buffer.len()]);
        } else {
            let digits = self.large().to_power_of_2_digits_asc(8);
            buffer[..digits.len()].copy_from_slice(&digits);
            buffer[digits.len()..].fill(0);
        }
    }

    #[inline(always)]
    pub fn to_ne_bytes(&self, buffer: &mut [u8]) {
        if cfg!(target_endian = "big") {
            self.to_be_bytes(buffer)
        } else {
            self.to_le_bytes(buffer)
        }
    }
...
impl fmt::Debug for BitVec {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut representation = f.debug_struct("BitVec");
        if self.is_small() {
            representation.field("value", &self.small());
        } else {
            representation.field("value", self.large());
        }
        representation
...
impl<'de> serde::Deserialize<'de> for BitVec {
    ...
    where
        D: Deserializer<'de>,
    {
        match Encoded::deserialize(deserialiser)? {
