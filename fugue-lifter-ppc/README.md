...
additionally bundle the `64-32addr` variant, which truncates the `ram` space
to 32-bit addresses. To generate lifters from source, use the following:

```sh
cargo build --features=compiled,ppc-be,ppc-le,ppc64-be,ppc64-le --no-default-features
```
