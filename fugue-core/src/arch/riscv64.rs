...
impl ArchData {
    fn new(language: &'static Language) -> Self {
        ...
        .map(|name| {
            reg(name).unwrap_or_else(|| panic!("RISCV language must define register `{name}`"))
        });
...
impl ArchT for RiscV64 {
    ...
    fn external_function_template(&self) -> ExternFunctionTemplate {
        ExternFunctionTemplate::new([0x67, 0x80, 0x00, 0x00])
    }
...
impl RiscV64 {
    ...
    pub fn resolve_variant_with<'a>(
        ...
        let lid = LanguageId::new_with("RISCV", false, 64, variant);
        Ok(loader.load(&lid)?)
    }
}
...
impl ArchProvider {
    const NAME: &str = "riscv64";

    fn supports(language: &'static Language) -> bool {
        language.processor() == "RISCV" && language.bits() == 64
    }

    fn create(language: &'static Language) -> Arch {
        RiscV64::new(language)
    }
}
...
impl LanguageProvider {
    ...
    fn provide(
        ...
        if id.processor() != "RISCV" || id.bits() != 64 {
            return Ok(None);
        }

        let language = match source.loader() {
            Some(loader) => RiscV64::resolve_variant_with(loader, id.variant())?,
            None => RiscV64::resolve_variant(id.variant())?,
        };
