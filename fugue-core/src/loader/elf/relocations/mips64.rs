...
use crate::loader::elf::relocations::mips::mips_implicit_addend;
    ...
    'file: 'data,
{
    pub(crate) fn apply_mips64_relocation(
        ...
        let Some(composite) = elf_relocation_type(reloc) else {
            return;
        };
        ...
        let implicit = if size == 8 {
            i64::from_ne_bytes(mips_implicit_addend::<u64>(bytes, offset, reloc).to_ne_bytes())
        } else {
            i64::from(mips_implicit_addend::<u32>(bytes, offset, reloc))
        };
