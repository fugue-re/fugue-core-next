...
use object::{Object, ReadRef, Relocation, RelocationKind};
    ...
    'file: 'data,
{
    ...
    pub(crate) fn apply_riscv_relocation(
        ...
        match reloc_type {
            ...
            R_RISCV_JUMP_SLOT | R_RISCV_32 | R_RISCV_64 => {
                let Some(symbol) = self.resolve_relocation_symbol(reloc, is_dynamic) else {
                    tracing::warn!("failed to resolve relocation {reloc_type:#x} at {offset:#x}");
                    return;
                };
                ...
                if reloc_type == R_RISCV_64 || (reloc_type == R_RISCV_JUMP_SLOT && self.elf.is_64())
                {
                    bytes.write_value(offset, value);
                } else {
                    if value > u32::MAX as u64 {
            ...
            R_RISCV_BRANCH => {
                let Some(value) =
                    self.riscv_displacement(bytes, offset, reloc, reloc_type, is_dynamic)
                else {
                    return;
                };
