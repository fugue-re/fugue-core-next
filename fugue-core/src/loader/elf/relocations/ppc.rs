use object::elf::{
    ...
    R_PPC_REL24, R_PPC_REL32, R_PPC_RELATIVE,
};
...
where
    ...
    'file: 'data,
{
    pub(crate) fn apply_ppc_branch_relocation(
        &self,
        ...
        let displacement = u32::try_from(value & i128::from(0x03ff_fffc))
            .expect("masked PowerPC branch displacement must fit in u32");
        bytes.update_value::<u32>(offset, |insn| (insn & !0x03ff_fffc) | displacement);
    }

    pub(crate) fn apply_ppc_half16_relocation(
        &self,
        bytes: &mut ImageSegmentContents<'data>,
        offset: u64,
        reloc: &Relocation,
        ...
        let Ok(value) = u32::try_from(symbol.wrapping_add_signed(reloc.addend())) else {
            tracing::warn!("relocation {reloc_type:#x} at {offset:#x} overflow");
            return;
        };
        ...
        let half = match half {
            ...
            Half16::Ha => {
                let bytes = value.wrapping_add(0x8000).to_le_bytes();
                u16::from_le_bytes([bytes[2], bytes[3]])
            }
        };
