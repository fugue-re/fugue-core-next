    ...
    pub(crate) fn apply_generic_relocation(
        ...
        match reloc_type {
            ...
            IMAGE_REL_BASED_HIGH => {
                ...
                let Some(current) = lsegm.read_value::<u16>(offset) else {
                ...
                lsegm.write_value(offset, current.wrapping_add(value));
            }
            IMAGE_REL_BASED_LOW => {
            ...
            IMAGE_REL_BASED_HIGHLOW => {
                ...
                let Some(current) = lsegm.read_value::<u32>(offset) else {
                    tracing::warn!("failed to read relocation {reloc_type:#x} at {offset:#x}");
                    return;
                };
            ...
            IMAGE_REL_BASED_DIR64 => {
                ...
                let Some(current) = lsegm.read_value::<u64>(offset) else {
                    tracing::warn!("failed to read relocation {reloc_type:#x} at {offset:#x}");
                    return;
                };
