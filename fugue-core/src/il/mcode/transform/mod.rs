...
impl IlTransformer for ECodeToMCode {
    ...
    fn transform(
        ...
        let mcode =
            ECodeToMCode::transform(self, source, context.arch(), context.platform(), None)?;
...
mod test {
    use fugue_bv::BitVec;
    use fugue_lifter::runtime::convention::{
        Convention, Prototype, PrototypeEntry, PrototypeOperand,
    };
    ...
    fn memory_domain_identifiers_round_trip_through_ecode_and_mcode() {
        ...
        let encoded = mcode
            ...
            .iter()
            .find(|operation| {
                operation.opcode() == MCodeOpcode::Undefined && operation.width() == 0
            })
    ...
    fn a_fixed_stack_slot_promotes_store_and_load() {
        ...
        assert!(
            !mcode.ops().iter().any(|operation| matches!(
                operation.opcode(),
                MCodeOpcode::Load | MCodeOpcode::Store
            ))
        );
    ...
    fn exact_stack_call_facts_materialise_stack_inputs_and_outputs() {
        ...
        let variable_output =
            IlValueId::try_from_index(materialisation.results().start() + 1).unwrap();
        let variable = mcode
    ...
    fn a_reused_transformer_does_not_retain_function_facts() {
        ...
        let first = transformer
            .transform(&first, &arch, &platform, Some(&facts))
            .unwrap();
        let second = transformer
            .transform(&second, &arch, &platform, None)
            .unwrap();
    ...
    fn transform_rejects_mismatched_and_out_of_range_function_facts() {
        ...
        let mismatch = transformer.transform(&source, &arch, &platform, Some(&mismatched));
        let range = transformer.transform(&source, &arch, &platform, Some(&out_of_range));
        let site = transformer.transform(&source, &arch, &platform, Some(&invalid_site));
