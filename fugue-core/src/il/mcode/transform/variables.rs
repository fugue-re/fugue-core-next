...
impl MCodeVariableWidths {
    pub(crate) fn new(
        ...
        for (index, value) in ir.values().iter().enumerate() {
            ...
            let Some(analysed) = analysis.variables().variable_for_value(source_value) else {
                continue;
            };
            widths.insert(variables[analysed.index()], value.width())?;
        }
        for (object_index, object) in analysis.stack().objects().iter().enumerate() {
            ...
            widths.insert(variables[analysed.index()], width)?;
        }
        Ok(widths)
    }
...
impl MCodeStackDefs {
    pub(crate) fn new(
        ...
        for (index, operation) in ir.ops().iter().enumerate() {
            ...
            let Some(access) = analysis.stack().access_for(site) else {
                continue;
            };
            let analysed = analysis
                .variables()
            ...
            if analysis.aliases().contains(analysed) {
                continue;
            }
...
impl MCodeCallOutputVariables {
    pub(crate) fn new(ir: &ECodeIr, analysis: &ECodeToMCodeAnalysis) -> Result<Self, IlError> {
        MCodeCallOutputSolver::new(ir, analysis).solve()
    }
...
impl<'a> MCodeCallOutputSolver<'a> {
    fn new(ir: &'a ECodeIr, analysis: &'a ECodeToMCodeAnalysis) -> Self {
        Self {
            source: ir,
            analysis,
            representatives: DisjointSet::new(analysis.variables().variables().len()),
    ...
    fn collect_call_output_at(
        ...
        if matches!(
            ...
            let outputs = self.analysis.abi().call(site).into_iter().flat_map(|call| {
        ...
        if operation.opcode() == ECodeOpcode::Undefined
            ...
            && let Some(variable) = self.analysis.variables().variable_for_value(value)
        {
            let output = *self
                .outputs
