...
use super::abi::{MCodeStorageFact, MCodeStorageLocation};
use crate::il::common::{IlArtefact, IlCsr, IlOpId, IlValueId, RegisterId};
use crate::il::ecode::ssa::{
    ECodeSsaBlockArgInputs, ECodeSsaDomain, ECodeSsaIr, ECodeSsaOpcode, ECodeSsaUses,
};
...
#[repr(transparent)]
pub(crate) struct MCodeStackObjectId(u32);
...
impl MCodeStackModel {
    pub(crate) fn new(
        ...
        let entry_stack_pointer = (0..ir.values().len())
            ...
            .find(|&value| {
                ir.value_domain(value) == Some(ECodeSsaDomain::Register(stack_pointer))
                    && ir
    ...
    fn collect_intervals(&self, ir: &ECodeSsaIr) -> Vec<StackObjectInterval> {
    ...
    fn collect_exposure(
        ...
        for index in 0..ir.values().len() {
            ...
            let exposed_here = uses.uses_for(value).iter().any(|site| {
                ...
                match user.opcode() {
                    ECodeSsaOpcode::Load | ECodeSsaOpcode::Store if site.operand_index() == 0 => {
                        false
                    }
                    ECodeSsaOpcode::Copy
    ...
    fn evaluate(
        ...
        if operation.opcode() == ECodeSsaOpcode::Undefined {
            return if ir.value_domain(value) != Some(ECodeSsaDomain::Register(stack_pointer)) {
        ...
        let affine = match operation.opcode() {
            ...
            ECodeSsaOpcode::Add => offset(0)
                .zip(displacement(1))
                .or_else(|| offset(1).zip(displacement(0)))
                .map(|(base, displacement)| base.shift(displacement)),
            ECodeSsaOpcode::Sub => offset(0).zip(displacement(1)).map(|(base, displacement)| {
