...
use crate::il::mcode::ssa::MCodeSsaOpcode;
use crate::ir::Address;
use crate::storage::segments::space::AddressSpaceId;

#[derive(Debug, Copy, Clone, PartialEq, Eq, rkyv::Archive, rkyv::Serialize, rkyv::Deserialize)]
#[rkyv(derive(Debug, PartialEq, Eq))]
pub struct MCodeSsaOp {
...
const _: () = assert!(size_of::<MCodeSsaOp>() <= 64);
...
impl MCodeSsaOp {
    ...
    pub const fn opcode(&self) -> MCodeSsaOpcode {
        self.opcode
    }
    ...
    pub(crate) fn constant(&self, constants: &[u8]) -> Option<BitVec> {
        if !matches!(self.opcode, MCodeSsaOpcode::Constant) {
            return None;
        }
        if self.width <= 64 {
    ...
    pub(crate) fn constant_bytes<'a>(&self, constants: &'a [u8]) -> Option<&'a [u8]> {
        if !matches!(self.opcode, MCodeSsaOpcode::Constant) || self.width <= 64 {
            return None;
        }
    ...
    pub(crate) fn replace_with_constant(&mut self, immediate: u64) {
        self.opcode = MCodeSsaOpcode::Constant;
